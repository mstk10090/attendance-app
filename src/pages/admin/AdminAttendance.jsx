...
const calcSplitDisplay = (item, shift) => {
  ...
  if (!item.clockOut) return `${item.clockIn} ~ (出勤中)`;
...
export default function AdminAttendance() {
  ...
  const getItemCategory = (item) => {
    ...
    if (item.clockIn && item.clockOut) {
      if (toMin(item.clockIn) > toMin(item.clockOut)) return "has_issue";
      if (calcWorkMin(item) <= 0) return "has_issue";
    }
    ...
    if (app.reason && (app.reason === "寝坊" || app.reason.includes("早退"))) return "has_issue";
  ...
  }, [items, filterName, filterStatus, filterLocation, filterDepartment, shiftMap, users]);
            ...
            <div style={{ marginBottom: "16px" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", paddingLeft: "78px" }}>
                {[
                  ...
                  { key: "no_issue", label: "問題なし", color: "#10b981" },
                  { key: "has_issue", label: "問題あり", color: "#f97316" },
                  { key: "resubmission", label: "再提出", color: "#a855f7" },
                  { key: "working", label: "出勤中", color: "#3b82f6" },
            ...
            <div ref={statusDropdownRef} style={{ position: "relative", display: "flex", alignItems: "center", gap: "4px" }}>
              ...
              >
                {(() => {
                  ...
                  const labels = { pending: "承認待ち", approved: "承認済み", no_issue: "問題なし", has_issue: "問題あり", working: "出勤中", incomplete: "未退勤", resubmission: "再提出", night: "深夜勤務", noshift: "未出勤", no_shift_day: "シフトなし", absent: "欠勤", day_off: "休み", sa_return_admin: "上位差戻(管)", sa_return_staff: "上位差戻(ス)" };
              ...
              {showStatusDropdown && (
                <div style={{
                  ...
                  {[{ key: "pending", label: "承認待ち" }, { key: "approved", label: "承認済み" }, { key: "no_issue", label: "問題なし" }, { key: "has_issue", label: "問題あり" }, { key: "working", label: "出勤中" }, { key: "incomplete", label: "未退勤" }, { key: "resubmission", label: "再提出" }, { key: "night", label: "深夜勤務" }, { key: "noshift", label: "未出勤" }, { key: "no_shift_day", label: "シフトなし" }, { key: "absent", label: "欠勤" }, { key: "day_off", label: "休み" }, { key: "sa_return_admin", label: "上位差戻(管)" }, { key: "sa_return_staff", label: "上位差戻(ス)" }].map(opt => (
                          ...
                          {group.records.map((item, idx) => {
                            ...
                            return (
                              <tr key={item.userId + item.workDate} style={{ background: bg }}>
                                ...
                                {idx === 0 && (
                                  <td rowSpan={group.records.length} style={{ fontWeight: "bold", fontSize: "14px", padding: "10px 8px", borderRight: "1px solid #d1d5db", borderBottom: "2px solid #e5e7eb", background: bg, verticalAlign: "middle" }}>
                                    {group.userName}
                ...
                <details style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: "10px", overflow: "hidden" }}>
                  ...
                  <div style={{ padding: "16px", borderTop: "1px solid #e5e7eb" }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "8px" }}>
                      {["遅刻・残業それぞれ理由を教えてください", "遅刻理由を教えてください", "残業理由を教えてください", "乖離理由を教えてください", "正しい勤怠時間で申請してください"].map(text => (
                        <button
                          key={text}
